...
pub async fn execute(a_game: &mut Game, launch_vertion: i64, must_delay: bool, delay: i64) {
    println!(" - {}", &a_game.transactions_location);
        //login
        ...
        send_exec(a_game).await;
        let _ = log_request_response(&a_game.transactions_location, &json!({"in": a_game.request.body,"out": a_game.response}));
...
fn set_start(a_game: &mut Game) {
    ...
    a_game.data.request_id = Uuid::new_v4().to_string();
    a_game.request.params.clear();
