...
pub async fn execute(a_game_name: String, a_location: String, must_delay: bool, delay: i64) -> Result<(), Box<dyn Error>> {
    ...
    let (l_url, cookies, referer) = match launch_vertion {
        1 => {network::launch_game_v1(l_launch_url.clone(), a_location.clone(), a_game_name.clone(), l_device.clone(), l_transactions_location.clone(), &mut default_data).await?},
        _ => {network::launch_game_v1(l_launch_url.clone(), a_location.clone(), a_game_name.clone(), l_device.clone(), l_transactions_location.clone(), &mut default_data).await?}
    };

    /*let jar = Arc::new(Jar::default());
    let cookies_json: Value = serde_json::from_str(&cookies).unwrap();

    for cookie in cookies_json.as_array().unwrap() {
        let name = cookie["name"].as_str().unwrap();
        let value = cookie["value"].as_str().unwrap();
        let domain = cookie["domain"].as_str().unwrap();
        let path = cookie["path"].as_str().unwrap_or("/");
        let scheme = if cookie["secure"].as_bool().unwrap_or(false) { "https" } else { "http" };
        let url = format!("{}://{}{}", scheme, domain.trim_start_matches('.'), path);
    ...
    match a_game_name.as_str() {
        "thor_hit_the_bonus" => {china_festival::execute(&mut game, launch_vertion, must_delay, delay).await; Ok(())}
