...
pub async fn send_exec(cmd_type: &str, a_game: &mut Game) {
    ...
    match cmd_type {
        ...
        "api" => {
            ...
            // Если есть заголовки, добавить их, иначе пустой объект
            // Пример, если нужен headers: data.insert("headers".to_string(), Value::Object(headers_map));
            // У вас пока не указаны отдельные headers, поэтому:
